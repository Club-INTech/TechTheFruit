...
class Dijkstra
	...
	def modifArrete(premier,second,distance)
                @arretes[premier][second]=distance
		@arretes[second][premier]=distance
	end
	...
	def chemin(n_depart,n_arrive,pas_intermediaire=true)
		t = parcours(n_depart,n_arrive).map!{ |numero| @noeuds[numero]}
		puts t.inspect
		if(!pas_intermediaire) then return rajouterPointPassage(t)
		else return enleverIntermediaires(t) end
	end
	...
	def parcours(n_depart,n_arrive)
		...
		while elemcourant!=n_depart
			...
			chemin.push(elemcourant)
                ...
                # @log.debug chemin.reverse.inspect.to_s
	...
	def enleverIntermediaires(chemin)
		...
		for i in (0 ..chemin.length-3)
			if(cosinus(Vecteur.new(chemin[i+1],chemin[i]),Vecteur.new(chemin[i+2],chemin[i+1]))<0.9)
				retour.push(chemin[i+1])
			end
		end
		retour.push(chemin.last)
		return retour
	end
