...
class Robot
        ...
        def initialize positionDebut = Position.new(0, 0, 0)
                @log = Logger.instance
                @tempsRestant = 90
	...
	#
        ...
        # Contrôle d'état des ultrason bas
        def etatBas
                @evitement.controleEtatBas
        end
        ...
        def goTo x, y, angle, *condition
                ...
                @asservissement.goTo Position.new(x, y, angle), :bypass
        end
        ...
        # Recalage du robot
        def recalage
                @asservissement.recalage2 :x, :negatif, 145
                goTo 400, position.y, Math::PI/2, :bypass
                @asservissement.recalage2 :y, :negatif, -145
