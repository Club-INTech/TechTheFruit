...
class Carte
	...
	def goTo(posA,posB,pas_intermediaire=true,lisser=false)
		...
		if(lisser)then a=Bspline.new(a).get
			...
			if(@graphe.cosinus(Vecteur.new(a[0],a[1]),Vecteur.new(a[1],a[2]))>0.98)
			a-=[a[1]]
			end
			taille=a.length-1
			if(@graphe.cosinus(Vecteur.new(a[taille],a[taille-1]),Vecteur.new(a[taille-1],a[taille-2]))>0.98)
			a-=[a[taille-1]]
			end
		end 	
		#a-=[a.first]
	...
	def bloquerZone(position,duree)
		...
		@graphe.arretes[zone].each_key{ |clef| 
			if(clef>24) then @graphe.arretes[clef].each_key{ |arrete| modifierArrete(clef,arrete,10000,duree)} end
			}
		end
		Thread.new {
		sleep(duree)
